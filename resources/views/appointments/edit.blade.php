        ...
        <div class="card-body">
            <form action="{{ route('appointments.update', $appointment) }}" method="POST" class="row g-3">
                ...
                <div class="col-md-6">
                    ...
                    <select name="customer_id" class="form-select @error('customer_id') is-invalid @enderror" required>
                        ...
                        @foreach($customers as $id => $name)
                            <option value="{{ $id }}" @selected(old('customer_id', $appointment->customer_id) == $id)>{{ $name }}</option>
                        @endforeach
                    </select>
                ...
                <div class="col-md-6">
                    ...
                    <select name="commercial_id" class="form-select @error('commercial_id') is-invalid @enderror" required>
                        <option value="">— Sélectionner —</option>
                        @foreach($commercials as $id => $name)
                            <option value="{{ $id }}" @selected(old('commercial_id', $appointment->commercial_id) == $id)>{{ $name }}</option>
                        @endforeach
                    </select>
