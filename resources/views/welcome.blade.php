...
<div class="container-fluid py-4 bg-light">
    ...
    <div class="d-flex flex-wrap justify-content-between align-items-center mb-4">
        ...
        <div class="d-flex gap-2">
            <div class="dropdown">
                <button class="btn btn-white bg-white border shadow-sm dropdown-toggle" type="button" data-bs-toggle="dropdown">
                    <i class="bi bi-plus-lg me-2"></i>Créer
                </button>
                <ul class="dropdown-menu dropdown-menu-end border-0 shadow">
                    <li><a class="dropdown-item" href="{{ route('appointments.create') }}"><i class="bi bi-calendar-event me-2 text-primary"></i>Rendez-vous</a></li>
                    <li><a class="dropdown-item" href="{{ route('products.create') }}"><i class="bi bi-person-plus me-2 text-success"></i>Client</a></li>
                </ul>
            </div>
                ...
                <div class="card-body p-3">
                    ...
                    <div class="mt-3">
                        <div class="progress" style="height: 4px;">
                            <div class="progress-bar bg-primary" style="width: {{ min(100, $kpis['rdv_today'] * 20) }}%"></div>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <div class="col-12 col-sm-6 col-xl-3">
            <div class="card border-0 shadow-sm h-100 card-hover-effect">
                <div class="card-body p-3">
                    ...
                    <div class="mt-3">
                        <div class="progress" style="height: 4px;">
                            <div class="progress-bar bg-info" style="width: {{ min(100, $kpis['rdv_week'] * 10) }}%"></div>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <div class="col-12 col-sm-6 col-xl-3">
            <div class="card border-0 shadow-sm h-100 card-hover-effect">
                <div class="card-body p-3">
                    ...
                    <div class="mt-3">
                        <div class="progress" style="height: 4px;">
                            <div class="progress-bar bg-success" style="width: {{ min(100, $kpis['clients_new'] * 10) }}%"></div>
                        </div>
                    </div>
                </div>
            </div>
                ...
                <div class="card-body p-3">
                    <div class="d-flex justify-content-between align-items-start">
                        <div>
                            ...
                            <div class="h2 fw-bold text-dark mt-2 mb-0">{{ number_format($kpis['revenue_week'], 0, ',', ' ') }} €</div>
                        </div>
                    ...
                    <div class="mt-3">
                        <div class="progress" style="height: 4px;">
                            <div class="progress-bar bg-warning" style="width: {{ min(100, ($kpis['revenue_week'] / 5000) * 100) }}%"></div>
                        </div>
                    </div>
                </div>
            </div>
                ...
                <div class="card-header bg-white py-3 border-0 d-flex justify-content-between align-items-center">
                    <div class="d-flex align-items-center gap-2">
                        ...
                        <div class="d-none d-md-flex gap-2 ms-3 small">
                            <span class="badge rounded-pill text-bg-success bg-opacity-10 text-success border border-success border-opacity-25">Confirmé</span>
                            <span class="badge rounded-pill text-bg-warning bg-opacity-10 text-warning border border-warning border-opacity-25">En attente</span>
                        </div>
                    </div>
        ...
        <div class="col-12 col-xl-3">
                ...
                <div class="card-body">
                    ...
                    <form action="#" method="GET">
                        <div class="input-group mb-2">
                            ...
                            <input type="search" class="form-control bg-light border-start-0" placeholder="Client, Facture...">
                        </div>
            ...
            <div class="card shadow-sm border-0">
                        ...
                        <div class="list-group-item border-0 px-3 py-2">
                            <div class="d-flex align-items-center">
                                <div class="flex-shrink-0 text-center rounded p-2 bg-light border" style="width: 50px;">
                                    <div class="small text-uppercase fw-bold text-muted">{{ \Carbon\Carbon::parse($rdv->start_at)->translatedFormat('M') }}</div>
                                    <div class="h5 mb-0 fw-bold">{{ \Carbon\Carbon::parse($rdv->start_at)->format('d') }}</div>
                                </div>
                                <div class="ms-3 flex-grow-1 overflow-hidden">
                                    <h6 class="mb-0 text-truncate" title="{{ $rdv->client_name }}">{{ $rdv->client_name }}</h6>
                                    <small class="text-muted d-block text-truncate">
                                        {{ \Carbon\Carbon::parse($rdv->start_at)->format('H:i') }} • {{ $rdv->subject ?? 'Rendez-vous' }}
                                    </small>
                                </div>
                            </div>
                        </div>
                ...
                <div class="card-footer bg-white text-center border-0">
                    <a href="{{ route('appointments.index') }}" class="small text-decoration-none fw-semibold">Voir tout le planning</a>
                </div>
            </div>
        </div>
    </div>
...
<style>
    ...
    #appointments-calendar {
        ...
        padding: 1rem;
        min-height: 600px; /* Plus haut par défaut */
    }
    ...
    .fc .fc-button-primary {
        ...
        font-weight: 600; text-transform: capitalize; padding: 0.4rem 1rem;
        box-shadow: 0 1px 2px rgba(0,0,0,0.05);
    }
    .fc .fc-button-primary:hover { background-color: #f8f9fa; border-color: #cdd4da; color: #212529; }
    .fc .fc-button-primary:not(:disabled).fc-button-active {
        background-color: var(--bs-primary); border-color: var(--bs-primary); color: white;
    }
    ...
    .fc .fc-day-today .fc-daygrid-day-number {
        background: var(--bs-primary); color: white; border-radius: 50%;
        width: 28px; height: 28px; display: flex; align-items: center; justify-content: center; margin: 4px;
    }

    /* Events Styling - Le cœur du changement */
    ...
    /* Contenu de l'event personnalisé */
    .fc-event-main-content {
        display: flex; align-items: center; gap: 6px; padding: 2px 4px; font-size: 0.8rem;
    }
    ...
    /* Couleurs spécifiques par statut (classes ajoutées via JS/PHP) */
...
{{-- Script pour la traduction française du calendrier --}}
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        ...
        // Helper pour déterminer la classe CSS selon le statut
        ...
        // Helper pour la couleur du badge modal
    ...
    @endforeach
        ...
        const calendar = new FullCalendar.Calendar(calendarEl, {
            ...
            eventContent: function(arg) {
                ...
                if (!timeText) {
                     // Fallback si timeText est vide (ex: vue mois parfois)
                ...
                // HTML personnalisé pour l'événement (Dot + Heure + Titre)
            ...
            // Action au clic : ouvrir la modale
            eventClick: function(info) {
                info.jsEvent.preventDefault(); // Empêche la navigation URL classique
                
                const props = info.event.extendedProps;
                const start = info.event.start;
                
                // Remplissage des données de la modale
        ...
        document.getElementById('btn-refresh-calendar').addEventListener('click', function() {
            // Ici tu pourrais appeler calendar.refetchEvents() si tu avais une source JSON
            // Pour l'instant on fait un reload simple
